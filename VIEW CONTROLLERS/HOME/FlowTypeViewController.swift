//
//  FlowTypeViewController.swift
//  Mzyoon
//
//  Created by QOL on 16/05/19.
//  Copyright © 2019 QOL. All rights reserved.
//

import UIKit

class FlowTypeViewController: CommonViewController,ServerAPIDelegate
{
    ...
    override func viewDidLoad()
    {
        ...
        if let language = UserDefaults.standard.value(forKey: "language") as? String
        {
            if language == "en"
            {
                self.navigationTitle.text = "ORDER FLOW TYPE"
                self.navigationTitle.transform = CGAffineTransform(scaleX: 1.0, y: 1.0)
            }
            else if language == "ar"
            {
                self.navigationTitle.text = "نوع التدفق"
                self.navigationTitle.transform = CGAffineTransform(scaleX: -1.0, y: 1.0)
            }
        }
        else
    ...
    @objc func tailorTypeButtonACtion(sender : UIButton)
    {
        if sender.tag == 1
        {
            Variables.sharedManager.flowTypeId = 1
        ...
        else
        {
            ...
            let tailorScreen = OrderTypeViewController()
            self.navigationController?.pushViewController(tailorScreen, animated: true)
        }
    }
